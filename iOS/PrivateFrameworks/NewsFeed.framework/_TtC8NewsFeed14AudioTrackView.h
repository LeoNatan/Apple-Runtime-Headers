//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

@interface _TtC8NewsFeed14AudioTrackView : UIView
{
    // Error parsing type: , name: thumbnailImageView
    // Error parsing type: , name: premiumBadgeView
    // Error parsing type: , name: publisherLogoImageView
    // Error parsing type: , name: publisherLabel
    // Error parsing type: , name: dragHandleImageView
    // Error parsing type: , name: titleLabel
    // Error parsing type: , name: trackDurationLabel
    // Error parsing type: , name: progressView
    // Error parsing type: , name: remainingTimeLabel
    // Error parsing type: , name: headphoneImageView
    // Error parsing type: , name: contentView
    // Error parsing type: , name: token
    // Error parsing type: , name: isDragHandleHidden
}

- (void).cxx_destruct;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;

@end

