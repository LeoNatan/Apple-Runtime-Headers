//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class PKPaymentProvisioningController, PKPaymentSetupFieldsModel, PKPaymentWebService;

@interface NPKPaymentProvisioningFlowController : NSObject
{
    id <NPKPaymentProvisioningFlowControllerDelegate> _delegate;
    PKPaymentWebService *_webService;
    PKPaymentProvisioningController *_provisioningController;
    unsigned int _currentStep;
    PKPaymentSetupFieldsModel *_fieldsModel;
}

@property(retain, nonatomic) PKPaymentSetupFieldsModel *fieldsModel; // @synthesize fieldsModel=_fieldsModel;
@property(nonatomic) unsigned int currentStep; // @synthesize currentStep=_currentStep;
@property(retain, nonatomic) PKPaymentProvisioningController *provisioningController; // @synthesize provisioningController=_provisioningController;
@property(retain, nonatomic) PKPaymentWebService *webService; // @synthesize webService=_webService;
@property(nonatomic) __weak id <NPKPaymentProvisioningFlowControllerDelegate> delegate; // @synthesize delegate=_delegate;
- (void).cxx_destruct;
- (void)_sendDidTransitionFromStep:(unsigned int)arg1 toStep:(unsigned int)arg2 withContext:(id)arg3;
- (void)_performTransitionToStep:(unsigned int)arg1 withContext:(id)arg2;
- (void)_endProvisioningFlowWithError:(id)arg1;
- (void)_transitionToSecondaryManualEntryWithFields:(id)arg1;
- (void)_transitionToManualEntry;
- (void)_transitionToRemoteCredentials:(id)arg1;
- (void)_handleFinished;
- (void)_performProvision;
- (void)_performTerms;
- (void)_performEligibility;
- (void)_requestRequirements;
- (id)_newPaymentProvisioningRequest;
- (id)_newPaymentEligibilityRequest;
- (id)_newPaymentRequirementsRequest;
- (void)_performNextActionForProvisioningState;
- (void)_handlePreconditionsVerified;
- (void)acceptTerms;
- (void)handleManualEntryFields:(id)arg1;
- (void)chooseManualEntry;
- (void)acknowledgeWelcome;
- (void)startProvisioningFlow;
- (id)initWithWebService:(id)arg1;

@end

