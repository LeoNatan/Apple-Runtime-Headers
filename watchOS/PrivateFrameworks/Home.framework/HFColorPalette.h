//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <Home/NAEquatable-Protocol.h>
#import <Home/NSCopying-Protocol.h>
#import <Home/NSMutableCopying-Protocol.h>

@class NSArray, NSString;

@interface HFColorPalette : NSObject <NAEquatable, NSCopying, NSMutableCopying>
{
    NSArray *_colors;
    unsigned int _paletteType;
}

+ (id)defaultColorPaletteOfType:(unsigned int)arg1;
+ (id)defaultColorPalette;
+ (id)_defaultColorsOfType:(unsigned int)arg1;
+ (id)_defaultTemperatureColors;
+ (id)_defaultRGBColors;
+ (id)warmWhiteColor;
- (void).cxx_destruct;
@property(readonly, nonatomic) unsigned int paletteType; // @synthesize paletteType=_paletteType;
@property(copy, nonatomic) NSArray *colors; // @synthesize colors=_colors;
- (_Bool)isEqual:(id)arg1;
@property(readonly) unsigned int hash;
- (id)colorPaletteByAdjustingForColorProfile:(id)arg1;
- (id)serializedRepresentation;
- (id)initWithSerializedRepresentation:(id)arg1 type:(unsigned int)arg2;
- (id)mutableCopyWithZone:(struct _NSZone *)arg1;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)objectAtIndexedSubscript:(unsigned int)arg1;
- (id)init;
- (id)initWithColors:(id)arg1 type:(unsigned int)arg2;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) Class superclass;

@end

