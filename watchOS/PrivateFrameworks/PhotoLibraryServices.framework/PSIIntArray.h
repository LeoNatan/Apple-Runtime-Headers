//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@interface PSIIntArray : NSObject
{
    struct sqlite3_intarray *_elementArray;
    long long _staticElementBuffer[256];
    long long *_elementBuffer;
    unsigned int _elementBufferSize;
    _Bool _freeOnUnprepare;
}

- (void)unbind;
- (void)bindElements:(const long long *)arg1 numberOfElements:(unsigned int)arg2;
- (void)bindElements:(struct __CFArray *)arg1 range:(struct _NSRange)arg2;
- (void)bindElements:(struct __CFSet *)arg1;
- (void)_unprepare;
- (void)unprepare;
- (void)_prepareForNumberOfElements:(unsigned int)arg1;
- (void)prepareForNumberOfElements:(unsigned int)arg1;
@property(readonly, nonatomic) long long *elementBuffer;
- (void)dealloc;
- (id)initWithLabel:(id)arg1 database:(struct sqlite3 *)arg2;

@end

