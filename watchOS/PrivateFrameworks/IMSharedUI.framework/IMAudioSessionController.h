//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface IMAudioSessionController : NSObject
{
    _Bool _active;
    _Bool _dirty;
    unsigned int _options;
}

+ (id)queue;
+ (id)sharedInstance;
+ (void)__setSingleton__im:(id)arg1;
+ (id)__singleton__im;
@property unsigned int options; // @synthesize options=_options;
@property(getter=isDirty) _Bool dirty; // @synthesize dirty=_dirty;
@property(getter=isActive) _Bool active; // @synthesize active=_active;
- (void)configureAudioSessionWithOptions:(unsigned int)arg1;
- (void)setActive:(_Bool)arg1 options:(unsigned int)arg2 completion:(CDUnknownBlockType)arg3;
@property(readonly) _Bool shouldDuckOthers;
@property(readonly) _Bool shouldStopPlayingWhenSilent;
@property(readonly) _Bool shouldUseSpeaker;
- (void)deactivate;
- (void)activateWithOptions:(unsigned int)arg1 completion:(CDUnknownBlockType)arg2;

@end

