//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NSCopying.h"

@class NTKRelevanceProvider;

@interface NTKRelevanceProviderManagerUpdate : NSObject <NSCopying>
{
    NTKRelevanceProvider *_provider;
}

+ (id)scheduledUpdateForAllProvidersAtDate:(id)arg1;
+ (id)scheduledUpdateForProvider:(id)arg1 atDate:(id)arg2;
+ (id)immediateUpdateForAllProviders;
+ (id)immediateUpdateForProvider:(id)arg1;
- (void).cxx_destruct;
@property(readonly, nonatomic) _Bool allProviders;
@property(readonly, nonatomic) NTKRelevanceProvider *provider;
- (unsigned int)hash;
- (_Bool)isEqual:(id)arg1;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)initWithProvider:(id)arg1;

@end

