//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NTKUpNextElementDataSourceDelegate.h"

@class NSArray, NSMutableArray, NSMutableDictionary, NSMutableSet, NSObject<OS_dispatch_queue>, NSRecursiveLock, NSString, NTKUpNextElementDataSource, NTKUpNextScheduler;

@interface NTKUpNextElementDataSourceController : NSObject <NTKUpNextElementDataSourceDelegate>
{
    NSString *_loggingHeader;
    NSMutableSet *_elements;
    NSMutableDictionary *_elementSectionMap;
    NSMutableDictionary *_elementIdentifierMap;
    NSRecursiveLock *_elementIdentifierMapLock;
    NSObject<OS_dispatch_queue> *_queue;
    NSMutableArray *_updates;
    NTKUpNextScheduler *_updateScheduler;
    NTKUpNextScheduler *_reloadScheduler;
    _Bool _hasUnlockedSinceFirstBoot;
    _Bool _wantsReloadWhilePaused;
    _Bool _isPerformingReload;
    NSMutableArray *_enqueuedBlocks;
    _Bool _willUnload;
    _Bool _allowsLocationUse;
    Class _dataSourceClass;
    NTKUpNextElementDataSource *_dataSource;
    unsigned int _state;
    id <NTKUpNextElementDataSourceControllerDelegate> _delegate;
}

+ (id)_sharedDataSourceQueue;
@property(nonatomic) __weak id <NTKUpNextElementDataSourceControllerDelegate> delegate; // @synthesize delegate=_delegate;
@property(nonatomic) unsigned int state; // @synthesize state=_state;
@property(readonly, nonatomic) NTKUpNextElementDataSource *dataSource; // @synthesize dataSource=_dataSource;
@property(readonly, nonatomic) Class dataSourceClass; // @synthesize dataSourceClass=_dataSourceClass;
- (void).cxx_destruct;
- (id)loggedState;
- (id)logName;
- (void)_loadLoggingHeader;
@property(readonly, nonatomic) NSString *bundleIdentifier;
@property(readonly, nonatomic) NSArray *allElements;
- (id)_elementsForIds:(id)arg1;
- (_Bool)isElementWithIdVisible:(id)arg1;
- (_Bool)hasElementWithId:(id)arg1 inSection:(unsigned int)arg2;
- (void)elementDidBecomeHidden:(id)arg1;
- (void)elementWillBecomeVisible:(id)arg1;
- (void)_queue_processUpdates:(id)arg1 forSection:(unsigned int)arg2;
- (void)_queue_performUpdates;
- (void)_queue_scheduleUpdate:(id)arg1;
- (id)_groupElements:(id)arg1 bySections:(id)arg2;
- (void)_queue_reload;
- (void)_performOrEnqueueUpdateBlock:(CDUnknownBlockType)arg1;
- (void)removeElementsWithIds:(id)arg1;
- (void)removeElements:(id)arg1 fromSection:(unsigned int)arg2;
- (void)refreshElement:(id)arg1;
- (void)refreshElement:(id)arg1 inSection:(unsigned int)arg2;
- (void)reloadElement:(id)arg1;
- (void)reloadElement:(id)arg1 inSection:(unsigned int)arg2;
- (void)reloadElement:(id)arg1 withElementId:(id)arg2 inSection:(unsigned int)arg3;
- (void)addElements:(id)arg1 toSection:(unsigned int)arg2;
- (void)invalidateElements;
- (id)elementOperationQueue;
- (void)prepareToUnload;
- (void)_queue_updateDataSourceLocationUse;
- (void)setAllowsLocationUse:(_Bool)arg1;
- (void)pause;
- (void)resume;
- (void)_handleDeviceLockStateChange:(id)arg1;
- (void)_handleSignifiantTimeChange:(id)arg1;
- (void)dealloc;
- (id)initWithDataSourceClass:(Class)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned int hash;
@property(readonly) Class superclass;

@end

