//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <Foundation/NSError.h>

@interface NSError (GEOError)
+ (id)geo_insufficientFilesystemCapacityWithBytesNeeded:(unsigned long)arg1 bytesAvailable:(unsigned long)arg2;
+ (id)GEOErrorWithCode:(int)arg1 userInfo:(id)arg2;
+ (id)GEOErrorWithCode:(int)arg1 reason:(id)arg2 underlyingError:(id)arg3;
+ (id)GEOErrorWithCode:(int)arg1 reason:(id)arg2;
+ (id)zilchDecoderErrorForNoSolution;
+ (id)_geo_errorFromXPCError:(id)arg1;
+ (id)_geo_errorFromXPCData:(id)arg1;
- (id)zilchDecoderTileLoadingError;
- (id)_geo_newXPCData;
@property(readonly, nonatomic) _Bool _geo_isXPCInterruptedError;
@end

