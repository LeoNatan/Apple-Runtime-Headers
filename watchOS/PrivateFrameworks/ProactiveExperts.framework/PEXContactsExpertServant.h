//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import <ProactiveExperts/PEXServant.h>

@class CNContactStore, NSArray, NSCache, NSObject<OS_dispatch_semaphore>, NSString, PEXContact, PEXContactScorer;

@interface PEXContactsExpertServant : PEXServant
{
    CNContactStore *_store;
    PEXContact *_meContact;
    NSCache *_meQuickTypeItemCache;
    NSArray *_peopleKeysToFetch;
    NSArray *_namesKeysToFetch;
    NSString *identifier;
    NSCache *_cachedNameLookups;
    PEXContactScorer *_contactScorer;
    NSObject<OS_dispatch_semaphore> *_initializationComplete;
    _Bool _consultScorerForImmediateResult;
}

+ (id)_supportedPeopleSemanticTypes;
- (void).cxx_destruct;
- (id)_selfContactQueryCriteriaFromSemanticTagCriteria:(id)arg1;
- (_Bool)_isSemanticTagEligible:(unsigned char)arg1;
- (id)_lookupPeopleWithNamePrefixUncached:(id)arg1 andRecipients:(id)arg2;
- (id)_lookupPeopleWithNamePrefix:(id)arg1 andRecipients:(id)arg2;
- (id)_lookupScoredPeopleWithNamePrefix:(id)arg1 andRecipients:(id)arg2;
- (id)_predictMeCardDetailForCriteria:(id)arg1 limit:(unsigned int)arg2 fromSemanticTextField:(_Bool)arg3;
- (id)_predictMeCardDetailForCriteria:(id)arg1 limit:(unsigned int)arg2;
- (id)_mePredictionCacheKeyForCriteria:(id)arg1;
- (_Bool)_isMeCardQuery:(id)arg1;
- (id)_predictionForPeopleCriteria:(id)arg1 fromEligibleSemanticTextField:(_Bool)arg2 limit:(unsigned int)arg3;
- (id)_applySmartLimitingToCandidates:(id)arg1 clientLimit:(unsigned int)arg2;
- (id)_predictionForNamesWithLimit:(unsigned int)arg1;
- (id)_nameItemsForAllContacts;
- (void)_clearChangeHistory:(id)arg1;
- (id)_contactsChangeHistoryWithError:(id *)arg1;
- (unsigned char)loadRecentNamesWithNamedEntityHandler:(CDUnknownBlockType)arg1 contactNameItemHandler:(CDUnknownBlockType)arg2 source:(unsigned char)arg3 objects:(id)arg4;
- (unsigned char)loadNamesWithNamedEntityHandler:(CDUnknownBlockType)arg1 contactNameItemHandler:(CDUnknownBlockType)arg2 detectedSince:(id)arg3;
- (id)nameItemsWithLimit:(unsigned int)arg1;
- (id)quickTypeItemsForCriteria:(id)arg1 limit:(unsigned int)arg2;
- (void)_preloadContacts;
- (void)_registerForNotifications;
- (void)setContactScorer:(id)arg1;
- (void)setCachedNameLookup:(id)arg1;
- (void)setStore:(id)arg1;
- (void)_warmUpFormatters;
- (id)initWithOptions:(unsigned char)arg1;
- (id)init;
- (void)clearCaches;
- (id)uncachedPredictionForNamesWithLimit:(unsigned int)arg1;

@end

