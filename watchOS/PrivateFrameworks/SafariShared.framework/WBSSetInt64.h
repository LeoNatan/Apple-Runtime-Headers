//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "NSCoding.h"
#import "NSCopying.h"
#import "NSSecureCoding.h"

@interface WBSSetInt64 : NSObject <NSCoding, NSSecureCoding, NSCopying>
{
    unordered_set_33a3a5d1 _set;
}

+ (_Bool)supportsSecureCoding;
- (id).cxx_construct;
- (void).cxx_destruct;
- (id)description;
- (id)commaSeparatedValues;
@property(readonly, nonatomic) unsigned long count;
- (_Bool)contains:(long long)arg1;
@property(readonly, nonatomic) const unordered_set_33a3a5d1 *set;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (id)copy;
- (void)encodeWithCoder:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (id)initWithMovableSet:(unordered_set_33a3a5d1 *)arg1;
- (id)initWithSet:(const unordered_set_33a3a5d1 *)arg1;
- (id)initWithInt64:(long long)arg1;
- (id)init;

@end

