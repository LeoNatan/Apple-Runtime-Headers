//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSObject<OS_dispatch_queue>, NSURL;

@interface WBSHistoryDatabaseAccessBroker : NSObject
{
    NSURL *_historyDatabaseURL;
    NSObject<OS_dispatch_queue> *_writeRequestQueue;
}

- (void).cxx_destruct;
- (id)databaseLastModifiedDate;
- (id)_openDatabaseWithAccessType:(int)arg1 onQueue:(id)arg2 error:(id *)arg3;
- (void)_requestAccessType:(int)arg1 onQueue:(id)arg2 completionHandler:(CDUnknownBlockType)arg3;
- (void)_requestReadWriteAccessOnQueue:(id)arg1 completionHandler:(CDUnknownBlockType)arg2;
- (void)_requestReadOnlyAccessOnQueue:(id)arg1 completionHandler:(CDUnknownBlockType)arg2;
- (void)requestAccessType:(int)arg1 onQueue:(id)arg2 completionHandlerWithInvalidationHandler:(CDUnknownBlockType)arg3;
- (void)requestAccessType:(int)arg1 onQueue:(id)arg2 completionHandler:(CDUnknownBlockType)arg3;
- (id)initWithHistoryDatabaseURL:(id)arg1;
- (id)init;

@end

