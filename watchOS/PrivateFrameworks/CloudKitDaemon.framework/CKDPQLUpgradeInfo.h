//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

__attribute__((visibility("hidden")))
@interface CKDPQLUpgradeInfo : NSObject
{
    _Bool _shouldVacuum;
    _Bool _shouldTruncateDatabase;
    unsigned int _version;
    CDUnknownFunctionPointerType _upgradeFunction;
}

+ (id)upgradeInfoWithVersion:(unsigned int)arg1 function:(CDUnknownFunctionPointerType)arg2 shouldVacuum:(_Bool)arg3 shouldTruncate:(_Bool)arg4;
@property(nonatomic) _Bool shouldTruncateDatabase; // @synthesize shouldTruncateDatabase=_shouldTruncateDatabase;
@property(nonatomic) _Bool shouldVacuum; // @synthesize shouldVacuum=_shouldVacuum;
@property(nonatomic) CDUnknownFunctionPointerType upgradeFunction; // @synthesize upgradeFunction=_upgradeFunction;
@property(nonatomic) unsigned int version; // @synthesize version=_version;
- (id)initWithVersion:(unsigned int)arg1 function:(CDUnknownFunctionPointerType)arg2 shouldVacuum:(_Bool)arg3 shouldTruncate:(_Bool)arg4;

@end

