//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import <BulletinBoard/BBBulletinUpdate.h>

@interface BBBulletinAddUpdate : BBBulletinUpdate
{
    _Bool _shouldPlayLightsAndSirens;
}

+ (_Bool)supportsSecureCoding;
+ (id)updateWithBulletin:(id)arg1 feeds:(unsigned int)arg2 shouldPlayLightsAndSirens:(_Bool)arg3;
@property(readonly, nonatomic) _Bool shouldPlayLightsAndSirens; // @synthesize shouldPlayLightsAndSirens=_shouldPlayLightsAndSirens;
- (id)initWithCoder:(id)arg1;
- (void)encodeWithCoder:(id)arg1;
- (id)description;
- (_Bool)isEqual:(id)arg1;
- (unsigned int)hash;
- (id)typeDescription;
- (id)initWithBulletin:(id)arg1 feeds:(unsigned int)arg2 shouldPlayLightsAndSirens:(_Bool)arg3;

@end

