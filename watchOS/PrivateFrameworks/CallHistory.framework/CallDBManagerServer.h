//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <CallHistory/CallDBManager.h>

@interface CallDBManagerServer : CallDBManager
{
}

+ (id)getObjectIdsForAllRecords:(id)arg1;
+ (id)getUniqueIdsForAllRecords:(id)arg1;
+ (id)getObjectIdsForDuplicateRecordsWithUniqueIds:(id)arg1 andHavingObjectIds:(id)arg2;
+ (id)downgradeDatabaseAtLocation:(id)arg1 toVersion:(int)arg2;
+ (id)getDestinationModel:(int)arg1;
+ (int)getNextVersionToDowngradeTo:(int)arg1 withSourceVersion:(int)arg2;
+ (int)getNextVersionToDowngradeTo:(int)arg1;
+ (int)getNextVersionToMigrateToCurrentVersion:(int)arg1;
+ (id)getDestinationModelForVersion:(int)arg1;
- (_Bool)bootUpDBAtLocation:(id)arg1 isEncrypted:(_Bool)arg2;
- (_Bool)createCallDBProperties;
- (void)prepareForMigrationDBAtURL:(id)arg1 withModelAtURL:(id)arg2 andIsEncrypted:(_Bool)arg3;
- (id)getUUIDsOfNMostRecentRecords:(unsigned int)arg1 fromManagedObjectContext:(id)arg2;
- (_Bool)handleDatabaseMigration:(id)arg1 isEncrypted:(_Bool)arg2 isRetry:(_Bool)arg3;
- (void)prepareDatabaseForNextStepInMigration:(id)arg1 dbVersion:(int)arg2 isEncrypted:(_Bool)arg3;
- (void)populateHandleType:(id)arg1 dbVersion:(int)arg2 isEncrypted:(_Bool)arg3;
- (void)populateRecentCallRemoteParticipantHandles:(id)arg1 dbVersion:(int)arg2 isEncrypted:(_Bool)arg3;
- (void)populateServiceProviderAndCallCategory:(id)arg1 dbVersion:(int)arg2 isEncrypted:(_Bool)arg3;
- (void)modifyCallRecordForDBAtLocation:(id)arg1 dbVersion:(int)arg2 isEncrypted:(_Bool)arg3 modifyCallRecord:(CDUnknownBlockType)arg4;
- (_Bool)removeDuplicatesFromDBAtLocation:(id)arg1 dbVersion:(int)arg2 isEncrypted:(_Bool)arg3;
- (id)createMOCForDBAtLocation:(id)arg1 dbVersion:(int)arg2 isEncrypted:(_Bool)arg3;
- (_Bool)handleBootUpFailure:(id)arg1;
- (_Bool)bootUpDBAtLocationWithRecovery:(id)arg1 isEncrypted:(_Bool)arg2;
- (void)moveCallsFromTempDatabase;
- (id)permDBLocation:(unsigned char *)arg1;
- (id)tempDBLocation:(unsigned char *)arg1;
- (void)createTemporary;
- (void)createPermanent;

@end

