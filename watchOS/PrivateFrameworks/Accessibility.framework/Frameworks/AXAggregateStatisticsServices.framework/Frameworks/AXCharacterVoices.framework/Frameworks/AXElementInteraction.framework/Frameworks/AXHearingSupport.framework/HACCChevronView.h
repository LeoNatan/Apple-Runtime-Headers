//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

@class UIColor;

@interface HACCChevronView : UIView
{
    UIView *_leftGrabberView;
    UIView *_rightGrabberView;
    double _animationDuration;
    int _state;
    UIColor *_color;
    float _alphaComponent;
    UIView *_tintView;
    UIView *_backgroundView;
    _Bool _unified;
    UIView *_alphaContainerView;
}

@property(nonatomic) int state; // @synthesize state=_state;
@property(retain, nonatomic) UIColor *color; // @synthesize color=_color;
- (void).cxx_destruct;
- (id)description;
- (void)setState:(int)arg1 animated:(_Bool)arg2;
- (_Bool)_setUnified:(_Bool)arg1;
- (_Bool)_setState:(int)arg1;
- (void)layoutSubviews;
- (void)_layoutGrabberView:(id)arg1 forState:(int)arg2;
- (struct CGRect)_frameForGrabberView:(id)arg1 forState:(int)arg2 unified:(_Bool)arg3;
- (struct CGAffineTransform)_transformForGrabberView:(id)arg1 forState:(int)arg2;
- (struct CGSize)sizeThatFits:(struct CGSize)arg1;
- (void)configureForLightStyle;
@property(nonatomic) double animationDuration;
- (id)initWithColor:(id)arg1;
- (id)initWithFrame:(struct CGRect)arg1;

@end

