//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@interface NSObject (NAAdditions)
- (void)_na_removeNotificationObserverProxy:(id)arg1;
- (void)_na_addNotificationObserverProxy:(id)arg1;
- (id)na_associatedObjectForDescriptor:(const CDStruct_3a9211e1 *)arg1;
- (void)na_setAssociatedObject:(id)arg1 assocatedObjectDescriptor:(const CDStruct_3a9211e1 *)arg2;
- (void)na_removeNotificationBlockObserver:(id)arg1;
- (id)na_addNotificationBlockObserverForObject:(id)arg1 keyPath:(id)arg2 options:(unsigned int)arg3 usingBlock:(CDUnknownBlockType)arg4;
- (id)na_addNotificationBlockObserverForName:(id)arg1 object:(id)arg2 queue:(id)arg3 usingBlock:(CDUnknownBlockType)arg4;
@end

