//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "FIDictionaryRepresentable.h"

@class NSArray, NSString, NSUUID;

@interface FIIntervalWorkoutDefinition : NSObject <FIDictionaryRepresentable>
{
    NSString *_name;
    NSUUID *_identifier;
    NSArray *_sets;
}

+ (id)intervalWorkoutDefinition;
+ (id)definitionFromDictionary:(id)arg1 error:(id *)arg2;
@property(readonly, nonatomic) NSArray *sets; // @synthesize sets=_sets;
@property(readonly, nonatomic) NSUUID *identifier; // @synthesize identifier=_identifier;
@property(readonly, nonatomic) NSString *name; // @synthesize name=_name;
- (void).cxx_destruct;
- (id)byRemovingIntervalDefinitionAtIndexPath:(id)arg1;
- (id)byReplacingIntervalDefinitionAtIndexPath:(id)arg1 withIntervalDefinition:(id)arg2;
- (id)byInsertingIntervalDefinition:(id)arg1 atIndexPath:(id)arg2;
- (id)byAddingIntervalDefinition:(id)arg1 toSetIndex:(unsigned int)arg2;
- (id)byRemovingSetDefinitionAtIndex:(unsigned int)arg1;
- (id)byReplacingSetDefinitionAtIndex:(unsigned int)arg1 withSetDefinition:(id)arg2;
- (id)byInsertingSetDefinition:(id)arg1 atIndex:(unsigned int)arg2;
- (id)byAddingSetDefinition:(id)arg1;
- (id)byChangingName:(id)arg1;
- (id)dictionaryRepresentation;
- (id)intervalDefinitionForIntervalAtIndex:(int)arg1;
@property(readonly, nonatomic) unsigned int intervalCount; // @dynamic intervalCount;
- (id)initWithName:(id)arg1 identifier:(id)arg2 sets:(id)arg3;

@end

