//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import <Metal/MTLComputePipelineReflection.h>

@class NSArray, NSDictionary;

__attribute__((visibility("hidden")))
@interface MTLComputePipelineReflectionInternal : MTLComputePipelineReflection
{
    NSArray *_arguments;
    NSArray *_builtInArguments;
    NSDictionary *_performanceStatistics;
    CDStruct_596dc0d1 _flags;
}

- (id)description;
- (id)formattedDescription:(unsigned int)arg1;
- (id)builtInArguments;
- (id)arguments;
- (CDStruct_596dc0d1)usageFlags;
- (void)setPerformanceStatistics:(id)arg1;
- (id)performanceStatistics;
- (void)dealloc;
- (id)initWithSerializedData:(id)arg1 device:(id)arg2 options:(unsigned int)arg3 flags:(CDStruct_596dc0d1)arg4;
- (id)initWithSerializedData:(id)arg1 serializedStageInputDescriptor:(id)arg2 device:(id)arg3 options:(unsigned int)arg4 flags:(CDStruct_596dc0d1)arg5;

@end

