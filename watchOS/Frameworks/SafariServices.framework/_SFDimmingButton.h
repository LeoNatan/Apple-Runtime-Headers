//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "UIButton.h"

@interface _SFDimmingButton : UIButton
{
    float _normalImageAlpha;
    float _highlightedImageAlpha;
    double _highlightAnimationDuration;
}

@property(nonatomic) double highlightAnimationDuration; // @synthesize highlightAnimationDuration=_highlightAnimationDuration;
@property(nonatomic) float highlightedImageAlpha; // @synthesize highlightedImageAlpha=_highlightedImageAlpha;
@property(nonatomic) float normalImageAlpha; // @synthesize normalImageAlpha=_normalImageAlpha;
- (void)_updateImageAlpha;
- (void)layoutSubviews;
- (void)setHighlighted:(_Bool)arg1;

@end

