//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

@class NSLock, NSObject<OS_dispatch_queue>;

@interface MTKTextureUploader : NSObject
{
    id <MTLDevice> _device;
    id <MTLCommandBuffer> _commandBuffer;
    id <MTLBlitCommandEncoder> _blit;
    NSLock *_lock;
    NSObject<OS_dispatch_queue> *_notifyQueue;
}

- (void)finishWithCompletionHandler:(CDUnknownBlockType)arg1;
- (id)newTextureWithData:(id)arg1 options:(id)arg2;
- (void)copyTexture:(id)arg1 toTexture:(id)arg2;
- (void)copyBytes:(const void *)arg1 toTexture:(id)arg2 bitsPerPixel:(unsigned int)arg3 pixelComponents:(unsigned int)arg4 bytesPerRow:(unsigned int)arg5 bytesPerImage:(unsigned int)arg6 region:(CDStruct_44fb454a)arg7 slice:(unsigned int)arg8 level:(unsigned int)arg9 flipVertically:(_Bool)arg10;
- (void)generateMipmapsForTexture:(id)arg1;
- (void)dealloc;
- (id)initWithDevice:(id)arg1 commandQueue:(id)arg2 notifyQueue:(id)arg3;

@end

