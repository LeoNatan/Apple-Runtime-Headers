//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class NSImage, NSString;

__attribute__((visibility("hidden")))
@interface IKDeviceItem : NSObject
{
    NSString *deviceName;
    NSString *deviceUUID;
    NSImage *deviceIcon;
    long long deviceID;
    BOOL isCamera;
    BOOL isScanner;
    BOOL isRoot;
    BOOL isNetwork;
}

@property long long deviceID; // @synthesize deviceID;
@property(retain) NSImage *deviceIcon; // @synthesize deviceIcon;
@property BOOL isNetwork; // @synthesize isNetwork;
@property BOOL isRoot; // @synthesize isRoot;
@property BOOL isScanner; // @synthesize isScanner;
@property BOOL isCamera; // @synthesize isCamera;
@property(copy) NSString *deviceUUID; // @synthesize deviceUUID;
@property(copy) NSString *deviceName; // @synthesize deviceName;
- (id)description;
- (void)dealloc;

@end

