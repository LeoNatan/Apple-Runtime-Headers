//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <PAImagingCore/NSCopying-Protocol.h>

@class NSString;

@interface RKExportSettings : NSObject <NSCopying>
{
    BOOL _blackPointCompensation;
    BOOL _includeMetadata;
    BOOL _includeGPS;
    BOOL _includeIPTC;
    BOOL _includeFaceNames;
    BOOL _skipBurstMasterExport;
    BOOL _skipMasterVideoComplement;
    BOOL _skipMaster;
    int _sizeStyle;
    long long _imageFormat;
    double _imageQuality;
    long long _destinationPixelWidth;
    long long _destinationPixelHeight;
    double _scalePercentage;
    long long _colorSpace;
    unsigned long long _renderingIntent;
    NSString *_movieExportQuality;
}

@property(nonatomic) BOOL skipMaster; // @synthesize skipMaster=_skipMaster;
@property(nonatomic) BOOL skipMasterVideoComplement; // @synthesize skipMasterVideoComplement=_skipMasterVideoComplement;
@property(nonatomic) BOOL skipBurstMasterExport; // @synthesize skipBurstMasterExport=_skipBurstMasterExport;
@property(nonatomic) BOOL includeFaceNames; // @synthesize includeFaceNames=_includeFaceNames;
@property(nonatomic) BOOL includeIPTC; // @synthesize includeIPTC=_includeIPTC;
@property(nonatomic) BOOL includeGPS; // @synthesize includeGPS=_includeGPS;
@property(nonatomic) BOOL includeMetadata; // @synthesize includeMetadata=_includeMetadata;
@property(retain, nonatomic) NSString *movieExportQuality; // @synthesize movieExportQuality=_movieExportQuality;
@property(nonatomic) BOOL blackPointCompensation; // @synthesize blackPointCompensation=_blackPointCompensation;
@property(nonatomic) unsigned long long renderingIntent; // @synthesize renderingIntent=_renderingIntent;
@property(nonatomic) long long colorSpace; // @synthesize colorSpace=_colorSpace;
@property(nonatomic) double scalePercentage; // @synthesize scalePercentage=_scalePercentage;
@property(nonatomic) long long destinationPixelHeight; // @synthesize destinationPixelHeight=_destinationPixelHeight;
@property(nonatomic) long long destinationPixelWidth; // @synthesize destinationPixelWidth=_destinationPixelWidth;
@property(nonatomic) int sizeStyle; // @synthesize sizeStyle=_sizeStyle;
@property(nonatomic) double imageQuality; // @synthesize imageQuality=_imageQuality;
@property(nonatomic) long long imageFormat; // @synthesize imageFormat=_imageFormat;
- (id)description;
- (id)copyWithZone:(struct _NSZone *)arg1;
- (void)dealloc;
- (id)init;
- (BOOL)usePreviewResources;
- (id)exportFormat;
- (struct CGSize)sizeForVersion:(id)arg1;
- (double)scaleForVersion:(id)arg1;
- (id)archiveDictionary;
- (id)initWithArchiveDictionary:(id)arg1;
- (void)setToArchiveDictionary:(id)arg1;

@end

