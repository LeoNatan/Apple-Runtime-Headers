//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Oct 15 2018 10:31:50).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

#import <PAImaging/PAMultipleImageRenderResponse-Protocol.h>
#import <PAImaging/PASingleImageRenderResponse-Protocol.h>

@class NSArray, NSError, NSPointerArray, NSString, PACGImage;

@interface PAImageRenderResponse : NSObject <PASingleImageRenderResponse, PAMultipleImageRenderResponse>
{
    NSPointerArray *_images;
    NSError *_error;
}

@property(retain) NSError *error; // @synthesize error=_error;
- (void).cxx_destruct;
- (BOOL)isComplete;
@property(retain) PACGImage *image;
- (void)setImage:(id)arg1 atIndex:(unsigned long long)arg2;
- (id)imageAtIndex:(unsigned long long)arg1;
@property(readonly) unsigned long long imageCount;
@property(readonly) NSArray *images;
@property(readonly, copy) NSString *description;
- (id)initWithImageCount:(unsigned long long)arg1;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

